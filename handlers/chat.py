            ...
            try:
                async with aiohttp.ClientSession() as session:
                    async with session.post(url, headers=headers, json=payload) as resp:
                        if resp.status == 429:
                            logging.warning(f"⚠️ Key {api_key[:8]}... rate limited (429) for {model_name}. Trying next key...")
                            break # break the models loop to try next key
    ...
    for api_key in GOOGLE_API_KEYS:
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.post(url, headers=headers, json=payload) as resp:
                    if resp.status == 429:
                        logging.warning(f"⚠️ Key {api_key[:8]}... rate limited (429) for photo. Trying next key...")
                        continue
                    if resp.status == 200:
            ...
            logging.error(f"Photo analysis error on key {api_key[:8]}: {e}")
            continue
    ...
    for api_key in GOOGLE_API_KEYS:
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.post(url, headers=headers, json=payload) as resp:
                    if resp.status == 429:
                        logging.warning(f"⚠️ Key {api_key[:8]}... rate limited (429) for summary. Trying next key...")
                        continue
                    if resp.status == 200:
                        data = await resp.json()
                        if 'candidates' in data and data['candidates']:
            ...
            logging.error(f"Personality summarization error on key {api_key[:8]}: {e}")
            continue
