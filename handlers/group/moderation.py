...
async def is_ai_promotion(text: str) -> bool:
    """Uses keywords and then Gemini AI to detect hidden spam/promotions."""
    if len(text) < 20:
        return False
        
    # Heuristic check to save API calls
    suspicious_keywords = [
        "sell", "buy", "channel", "account", "website", "trusted", 
        "deal", "profit", "investment", "link", "join", "promotion", 
        "followers", "views", "subscribe", ".com", ".in", ".org", "dm me", "inbox"
    ]
    text_lower = text.lower()
    
    if not any(kw in text_lower for kw in suspicious_keywords):
        return False
    ...
    prompt = f"Analyze this Telegram message. Is it a spam advertisement, promotion, or selling service? Answer ONLY with 'YES' or 'NO'.\n\nMessage:\n{text}"
    
    payload = {
        "contents": [{"role": "user", "parts": [{"text": prompt}]}],
        "system_instruction": {"parts": [{"text": "You are a strict anti-spam bot. Reply ONLY with YES or NO."}]}
    }
