...
from handlers.group.settings import groupsetup_command, group_settings_callback, setwelcome_command, handle_topic_reply
...
async def main():
    ...
    # Chat member handler to detect join/leave + bot promotion
    application.add_handler(ChatMemberHandler(chat_member_updated, ChatMemberHandler.CHAT_MEMBER))
    application.add_handler(ChatMemberHandler(group_member_updated, ChatMemberHandler.MY_CHAT_MEMBER))
    ...
    # Handle reply to AI Context (Topic) setup prompt (group=-1 so message_handler still runs for non-topic replies)
    application.add_handler(MessageHandler(filters.ChatType.PRIVATE & filters.TEXT & ~filters.COMMAND, handle_topic_reply), group=-1)
